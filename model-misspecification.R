...
for (n in n.vec){
  for (i in 1:t){
    ...
    Ry = rbinom(n,1,expit(b0.ry+b1.ry*Rx+b2.ry*X+b3.ry*X^2)) # add a square term
    X[Rx==0] <- NA
    Y[Ry==0] <- NA
    table(Rx,Ry)
    ...
    # estimation
    ans <- gmm(f.est1,x=dt,t0=c(-1,1))
    index <- (which(n.vec==n)-1)*100+i
...
# save data
save(list = c("estimated","covariance"),file = "local_mis_GMM_bivariate_dat.Rdata")
...
# save data
save(list = c("estimated","covariance"),file = "./Known intercept/local_mis_GMM_bivariate_dat.Rdata")
...
############define the estimating equation###############n
f.est1 <- function(para,x) {
...
for (n in n.vec){
  ...
  for (i in 1:t){
    ...
    Y = rnorm(n,mu.y+rho*sig.y/sig.x*(X-mu.x),sqrt((1-rho^2))*sig.y)
    Rx = rbinom(n,1,expit(b0.rx+b1.rx*Y))
    Ry = rbinom(n,1,expit(b0.ry+b1.ry*Rx+b2.ry*X+b3.ry*X^2))
    X[Rx==0] <- NA
    Y[Ry==0] <- NA
...
# save data-----------------------------------------------------change here
save(list = c("estimated","covariance"),file = "local_mis_GMM_opt_est_bivariate_dat.Rdata")
...
# save data
save(list = c("estimated","covariance"),file = "./Known intercept/local_mis_GMM_opt_est_bivariate_dat.Rdata")
...
# save data
save(list = c("estimated","covariance"),file = "cluster_mis_order_statistic_bivariate_dat.Rdata")